...
def main():
    ...
    # 1. Normal dataset
    X1, _ = make_blobs(
        n_samples=200, centers=4, cluster_std=[1.0, 2.5, 0.5, 2.8], random_state=42
    )
    # 2. Various Cluster Size and Densities
    X2, _ = make_blobs(
        n_samples=[100, 200, 800],
        centers=[[0, 0], [10, 10], [5, -5]],
    ...
    X3 = np.vstack([X3, rng.uniform(low=-10, high=10, size=(40, 2))])  # Add outliers
    # 4. Non-Spherical
    X4, _ = make_circles(n_samples=500, factor=0.8, noise=0.1, random_state=55)
    datasets = [X1, X2, X3, X4]
    datasets_types = [
        "Normal",
        "Various Cluster Size and Densities",
        "Outliers",
        "Non-Spherical",
    ]
