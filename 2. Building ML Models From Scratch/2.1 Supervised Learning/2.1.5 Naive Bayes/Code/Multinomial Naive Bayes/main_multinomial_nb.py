...
def main():
    df = pd.read_csv("2. Building ML Models From Scratch/_datasets/spam.csv")
    df["clean_text"] = df["Message"].apply(HelperFuncs.clean_text)  # Clean data
    X, y = df["clean_text"], df["Category"]

    # Transform X in Bag of Words
    vectoriser = CountVectorizer()
    X_doc_term = vectoriser.fit_transform(X)
    X = pd.DataFrame(X_doc_term.toarray(), columns=vectoriser.get_feature_names_out())
